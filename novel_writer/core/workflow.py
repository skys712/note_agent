...
class WorkflowOrchestrator:
    ...
    # 仓库根: novel_writer/core/workflow.py -> 上三级
    ...
    # 领域生成顺序与所有领域常量从 ProjectContext.WORLD_DOMAINS 共享
    ...
    def _generate_domain(self, domain: str, premise: str, cross_refs: str,
                         fragment_summary: str = "", allow_invent: bool = False,
                         raw_fragments: str = "",
                         mem_lock: threading.Lock = None,
                         skip_index: bool = False) -> bool:
        """生成单个领域: WorldBuilder → Editor 审校 → (可选) 修订

        mem_lock: 线程锁，用于保护 agent memory 的并发写入
        skip_index: 跳过 world index 更新（并行模式下最后统一重建）
        """
        name = self.DOMAIN_NAMES.get(domain, domain)

        memory = self.ctx.read_agent_memory("world_builder")
        # 并行模式下跳过跨领域引用（每个领域都从碎片出发，互不依赖）
        ...
        if not result.success:
            print(f"  {prefix}生成失败: {result.error}")
            return False
        if not result.content.strip():
            print(f"  {prefix}生成内容为空 (LLM 可能未按格式输出), 跳过 {domain}")
            if mem_lock:
                with mem_lock:
        ...
        review = self.editor.execute(task, context=review_context, memory=memory)
        if review.success and "通过" not in review.content:
            print(f"  {prefix}审校意见: {review.content[:200]}...")
            # 修订
    ...
    def generate_world(self, premise: str = "", allow_invent: bool = False) -> None:
        ...
        # Step 0: 扫描碎片，生成参考摘要 + 收集原始碎片
        fragment_summary = ""
        raw_fragments = ""
        fragments = self.ctx.list_fragments()
        if fragments:
            self.log.section(f"碎片扫描: 检测到 {len(fragments)} 个碎片，正在生成参考摘要...")
            self._scan_fragments(verbose=False)
            fragment_summary = self.ctx.get_fragments_summary() or ""
            if fragment_summary:
                print(f"  碎片摘要已生成 ({len(fragment_summary)} 字符)，将作为世界观生成的基础参考。")

            # 收集原始碎片全文（128K-1M 上下文窗口，每个碎片最多20000字符）
            # 顶层设定/设定类碎片优先排在前面，作为权威参考
            ...
            raw_fragments = "\n\n".join(raw_parts)
            print(f"  原始碎片已收集 ({len(raw_fragments)} 字符)，将作为一级参考注入。")
        else:
            print("  未检测到碎片，将基于前提直接生成。")

        domains = self.WORLD_DOMAINS
        succeeded = 0
        mem_lock = threading.Lock()

        # 并行生成所有领域（每个领域从其领域提示+碎片出发，互不依赖）
    ...
    def refresh_character(self, char_id: str, specs: str = "") -> None:
        ...
        print(f"\n人物 {name} ({char_id}) 刷新完成!")
        self.log.summary()

    def create_characters_batch(self, chars: list[dict]) -> None:
        ...
        def _create_one(info: dict) -> tuple[str, str, str, str]:
            """在线程中创建单个人物，返回 (name, char_id, content, notes)"""
            name = info["name"]
            ...
            review = self.editor.execute(review_task)
            if review.success and "通过" not in review.content:
                # 修订
                ...
                revised = self.character.execute(revise_task, memory=memory)
                if revised.success and revised.content.strip():
                    result = revised
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(_create_one, c): c for c in chars}
            for future in as_completed(futures):
                info = futures[future]
                try:
                    name, char_id, role, faction = future.result()
                    if char_id:
                        succeeded += 1
                        print(f"  [OK] {name} ({char_id})")
                    else:
                        print(f"  [SKIP] {info['name']} (生成失败)")
                except Exception as e:
    ...
    def _get_story_constraints(self) -> str:
        ...
        查找顺序:
        1. <项目目录>/constraints.md（项目专属）
        2. <仓库根>/input_config/story_constraints.md（工作区通用）
        如果都不存在，从 _meta.md 的 logline 生成最小约束。
        """
        # 先尝试项目专属约束文件
    ...
    def generate_chapter_scenes(self, vol: int, ch: int) -> None:
        ...
        self.log.step_start(1, 1, "剧情编剧", "design")
        task = AgentTask(
            action="design",
            input_text=(
                f"{constraints}\n\n"
    ...
    def write_section(self, vol: int, ch: int, sec: int) -> None:
        ...
        # Step 1: 剧情编剧设计/刷新场景
        self.log.step_start(1, 14, "剧情编剧", "design")
        ...
        task = AgentTask(
            ...
            input_text=(
                ...
                f"{fragment_context}\n\n"
                f"## 设计前必读: 权威参考材料\n\n"
                f"**在开始场景设计之前，必须仔细阅读以下世界观设定和人物卡。"
                f"场景中的地点、出场人物、世界观元素都必须能在以下材料中找到依据。**\n\n"
                ...
                f"---\n\n"
                f"## 设计任务\n"
                f"为第{sec}节设计完整场景，标注目标/冲突/转折。\n\n"
                f"场景设计格式:\n"
                f"## 场景 (section_{sec:03d}): 场景标题\n"
                f"- POV: 视角人物 (必须在上方人物卡中存在)\n"
                f"- 地点: 具体地点 (必须在上方世界观设定中有记载)\n"
                f"- 出场人物: 人物名列表\n"
                f"- 世界观元素: [领域:具体元素] 每个元素必须可追溯到上方世界观设定\n"
                f"- 核心冲突: 一句话冲突描述\n"
                f"- 字数目标: 3000-5000\n\n"
                f"要求: 所有地点和世界观元素必须有设定依据，不得生造。"
            ),
        )
        plot_design = self.plot.execute(task, context=context, memory=memory)
        ...
        world_context = self.context_builder.build(self.ctx, "world_builder", vol, ch, sec, constraints=constraints)
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            world_future = executor.submit(
                self.world.execute, world_task, context=world_context, memory=world_memory
            )
            char_check = char_future.result()
            world_check = world_future.result()
        ...
        if char_issues or world_issues:
            ...
            revised = self.plot.execute(revise_task, context=revise_context,
                                        memory=self.ctx.read_agent_memory("plot_writer"))
            if revised.success and revised.content.strip():
                plot_design = revised
        ...
        emotion_guide = self.emotion.execute(task, context=context, memory=memory)
        if emotion_guide.success:
            self.ctx.write_agent_memory("emotion_controller", emotion_guide.notes)
            print(f"  情绪指导: {emotion_guide.content[:300]}")
        else:
            print(f"  情绪分析异常, 继续执行。")
        ...
        break_plan = self.chapter_break.execute(task, context=context, memory=memory)
        if break_plan.success:
            self.ctx.write_agent_memory("chapter_break_director", break_plan.notes)
            print(f"  断章策略: {break_plan.content[:300]}")
        else:
            print(f"  断章策略异常, 继续执行。")
        ...
        task = AgentTask(
            ...
            input_text=(
                ...
                f"### 人物卡（所有角色必须严格遵循）\n{char_summary}\n\n"
                f"---\n\n"
                f"## 本章场景设计（来自 _meta.md，最高优先级，必须严格遵循）\n{chapter_outline[:3000]}\n\n"
                f"---\n\n"
                f"## 本节场景设计（是本章设计的细化，必须严格遵循）\n{plot_design.content}\n\n"
                f"---\n\n"
                f"## 写作指导\n\n"
        ...
        while True:
            ...
            verify_feedback = self._run_verification_checks(
            ...
            if not verify_feedback:
                print(f"  {round_label}所有验证通过。" if verify_round > 1 else "  所有逆向验证通过。")
                break

            # 输出本轮所有验证问题和修改意见
            print(f"\n{'='*60}")
            print(f"  {round_label}验证: {len(verify_feedback)} 个 Agent 发现问题")
            print(f"{'='*60}")
            for i, (name, content) in enumerate(verify_feedback, 1):
                print(f"\n  [{i}] {name} -- 问题和修改意见:")
                print(f"{_indent(content, '      ')}")
            print(f"\n{'='*60}")
            ...
            print(f"\n  是否根据以上意见修订正文? (y/n): ", end="")
            choice = input().strip().lower()

            if choice not in ("y", "yes", "是"):
                print("  跳过修订，验证问题将记录到 state.md 供后续参考。")
                break

            print(f"  开始第 {verify_round} 轮修订...")
            feedback_text = "\n\n".join(
            ...
            context = self.context_builder.build(self.ctx, "style_executor", vol, ch, sec, constraints=constraints)
            memory = self.ctx.read_agent_memory("style_executor")
            revised = self.style.execute(revise_task, context=context, memory=memory)
            if revised.success and revised.content.strip():
                ...
                print(f"  修订后正文: {len(generated_text)} 字")
            else:
                print("  修订失败，保留当前正文。")
                break
        ...
        # Step 14: 状态记录员更新剧情状态
        self._update_state(vol, ch, sec, generated_text, all_verify_notes_str)
    ...
    def _update_state(self, vol: int, ch: int, sec: int, section_content: str,
    ...
    def _get_chapter_outline(self, vol: int, ch: int) -> str:
        ...
        vol_outline = self.ctx.get_volume_outline(vol)
        if not vol_outline:
            print(f"  [WARN] 第 {vol} 卷大纲尚未生成,第 {ch} 章无可参考的大纲。")
        ...
        if chapter_lines:
            return "\n".join(chapter_lines)
        print(f"  [WARN] 第 {vol} 卷大纲中未找到第 {ch} 章的标记,将使用卷大纲前 500 字作回退。")
        return f"第 {ch} 章 (大纲未明确指定本章)\n\n卷大纲摘要:\n{vol_outline[:500]}"


    def _run_verification_checks(self, generated_text: str, full_world: str,
        ...
        每个验证步骤的结果 notes 会写入对应 agent 的 memory 文件。
        """
        # 构建待执行的检查列表: (名称, agent, task)
        ...
        if direction and direction.success:
            checks.append(("总编", self.editor, AgentTask(
                ...
                input_text=(
                    f"## 你的写作指导\n{direction.content}\n\n"
                    f"## 实际生成的正文\n{generated_text[:3000]}\n\n"
                    "请检查正文是否遵循了你的写作指导。如果完全符合回复 [通过]，否则列出偏差。"
                ),
            )))
        ...
        checks.append(("剧情编剧", self.plot, AgentTask(
            ...
            input_text=(
                f"## 你的场景设计\n{plot_design.content}\n\n"
                f"## 实际生成的正文\n{generated_text[:3000]}\n\n"
                "请检查正文是否按照你的场景设计执行。如果完全符合回复 [通过]，否则列出偏差。"
            ),
        )))
        ...
        with ThreadPoolExecutor(max_workers=len(checks)) as executor:
            future_map = {
                executor.submit(agent.execute, task): name
                for name, agent, task in checks
            }
            for future in as_completed(future_map):
                ...
                if "通过" not in result.content:
                    ...
                    print(f"  {name}验证发现偏差: {result.content[:200]}")
                else:
                    print(f"  {name}验证通过。")
    ...
    def _merge_agent_memory(self, agent_id: str, new_notes: str) -> None:
        ...
        if not existing or "尚未开始" in existing:
            ...
            return
        # 提取新笔记中的 [ACTIVE] 和 [CONTRADICTION] 区块
    ...
    def _extract_note_blocks(text: str) -> list[tuple[str, str]]:
        """从笔记文本中提取 [ACTIVE]/[CONTRADICTION] 区块"""
        blocks = []
        current_kind = ""
        current_lines = []
